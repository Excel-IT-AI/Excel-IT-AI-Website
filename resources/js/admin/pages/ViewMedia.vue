<template>
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        ...
                        >
                            ...
                            <tbody>
                                <tr
                                    v-for="media in filteredEntries"
                                    :key="media.id"
                                >
                                    <td>{{ media.id }}</td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        <a
                                            href="#"
                                            @click.prevent="editMedia(media.id)"
    ...
    <AddMedia
        @addMedia="getAllMedias(), (showAddModal = false)"
        @hideAddMediaModal="showAddModal = false"
        v-if="showAddModal"
    ></AddMedia>
    <EditMedia
        @updatedMedia="getAllMedias(), (showEditModal = false)"
        @hideEditModal="showEditModal = false"
        v-if="showEditModal"
        :media_id="media_id"
    ></EditMedia>
</template>
...
import AddMedia from "./mediamodals/AddMedia.vue";
import EditMedia from "./mediamodals/EditMedia.vue";
import Swal from "sweetalert2";
import GetAdmin from "../mixins/GetAdmin";
export default {
    mixins: [GetAdmin],
    components: { AddMedia, EditMedia },
    data() {
        return {
            columns: [
                { name: "id", text: "Media Id" },
                { name: "media_image", text: "Media Image" },
                { name: "media_title", text: "Media Title" },
                { name: "media_link", text: "Media Link" },
                { name: "media_link_name", text: "Media Link Name" },
        ...
        getAllMedias() {
            MediaApi.lists()
                .then(({ data: { mediaList } }) => {
                    ...
                    this.paginateData(this.mediaList);
                })
                .catch((error) => {
                    console.log(error);
                });
    ...
    mounted() {
        this.GetAdmin();
        this.getAllMedias();
        document.title = "View Media List | Admin | Excel IT AI";
    },
};
</script>
